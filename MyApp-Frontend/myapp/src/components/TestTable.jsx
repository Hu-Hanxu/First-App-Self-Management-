...
const TestTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Axios.get('http://192.168.3.10:5000//api/tests');
        if (response.status === 200) {
          const data = response.data;
          if (Array.isArray(data.tests)) {
            setRows(data.tests); // 提取出数组部分
          } else {
  ...
  const addRow = async () => {
    if (!showActions) {
      ...
      try {
        const response = await Axios.post('http://192.168.3.10:5000//api/tests', newRow, {
          headers: {
        ...
        if (response.status === 200) {
          const data = response.data;
          console.log('数据已保存到后端！课题ID：', data.test_id);
          const updatedRows = [...rows];
          updatedRows[updatedRows.length - 1].test_id = data.test_id;
          setRows(updatedRows);
        } else {
  ...
  const deleteRow = async (index) => {
    ...
    try {
      const response = await Axios.delete(`http://192.168.3.10:5000//api/tests/${testId}`);
      if (response.status === 200) {

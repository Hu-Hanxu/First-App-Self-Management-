...
const SubjectTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Axios.get('http://192.168.3.10:5000//api/subjects');
  ...
  const addRow = async () => {
    if (!showActions) {
      ...
      try {
        const response = await Axios.post('http://192.168.3.10:5000//api/subjects', newRow, {
          headers: {
  ...
  const deleteRow = async (index) => {
    ...
    try {
      const response = await Axios.delete(`http://192.168.3.10:5000//api/subjects/${subjectId}`);
      if (response.status === 200) {
